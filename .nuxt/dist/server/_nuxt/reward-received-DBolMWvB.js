...
import { D as _sfc_main$1, n as _plugin_vue_export_helper_default, r as _sfc_main } from "../server.mjs";
...
var reward_received_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		* (mockScore ของระบบภารกิจ Mock เดิม) ตรง ๆ — ตอนนี้ระบบภารกิจเป็นของจริงที่
		* Backend เป็นเจ้าของคะแนนอยู่แล้ว จึงกลับไปใช้ composables/useRoundScores.ts
		* (เรียก action 'getRoundScores' สดทุกครั้ง) เหมือนที่ควรเป็นตั้งแต่แรก — ต้อง
		* ตรงกับที่ pages/round-summary.vue แสดงเป๊ะ ๆ เสมอ (ดึงจากแหล่งเดียวกัน)
		*
		* [แก้ไข] handleOk() สำเร็จแล้ว reset ข้อมูลเกมของรอบนี้ให้ครบ (เพิ่ม
		* clearStationQuestProgress() + clearStationMissionsProgress()) + ล้าง
		* Profile/Auth แล้วพาไป "/" (หน้า Login) แทน "/home" เดิม — จบเกมแล้วต้องกลับไป
		* เริ่มจาก Login เสมอ ไม่เปิด Round/คะแนนเดิมค้างมาให้ผู้เล่นคนถัดไปเห็น (ดู doc
		* comment ที่ handleOk() เองสำหรับเหตุผลเต็ม ๆ)
		*/
		...
		* โดยไม่มีคำเตือนซ้ำ (เหมือน pages/round-summary.vue ทุกประการ) */
		...
		onBeforeRouteLeave(() => {
			...
			return (void 0).confirm("คุณยังไม่ได้กด OK ยืนยันรับรางวัล — ต้องการออกจากหน้านี้เลยหรือไม่?");
		});
		/**
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (!unref(isReady)) _push(`<div class="loading" data-v-951f6e47><p class="loading__text" data-v-951f6e47>กรุณารอสักครู่...</p></div>`);
				...
				if (!unref(hasValidRound)) {
					...
					_push(`<div class="info-card__text" data-v-951f6e47><p class="info-card__label" data-v-951f6e47>รหัสรอบ (roundId)</p><p class="info-card__value" data-v-951f6e47>${ssrInterpolate(unref(roundSummary)?.roundId)}</p></div></div><div class="info-card__row" data-v-951f6e47>`);
					_push(ssrRenderComponent(_component_UIcon, {
...
//# sourceMappingURL=reward-received-DBolMWvB.js.map