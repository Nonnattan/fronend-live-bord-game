...
import { i as useAdventure } from "./useAdventure-oyoryhV9.js";
import { t as useRound } from "./useRound-BmAVHypg.js";
import { t as useRoundSummary } from "./useRoundSummary-C3F2b7Ya.js";
import { a as useOfflineAnswerSync, i as useQuestion } from "./useStationMissions-DyKPx7bx.js";
...
import { t as _sfc_main$3 } from "./Input-03a9B_yA.js";
import { Fragment, computed, createBlock, createCommentVNode, createTextVNode, createVNode, defineComponent, isRef, mergeProps, nextTick, openBlock, ref, renderList, toDisplayString, unref, useSSRContext, watch, withCtx, withKeys } from "vue";
...
var scan_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		*/
		...
		const { stations, isVisited, isComplete, toggleStation, initAdventure, totalPoint } = useAdventure();
		...
		const { initQuestions, initAnsweredState, getQuestionForStation, isStationQuestionDone, findAnswered, submitAnswer: submitQuestionAnswer, resetAnswered, totalQuestionPoints, totalCorrect: totalQuestionCorrect } = useQuestion();
		const { initOfflineAnswerSync, syncNow: syncAnswersNow } = useOfflineAnswerSync();
		const { startStationTimer, clearStationTimer, stationRemainingLabel, hasActiveRoundTimer, isRoundExpired, isStationExpired } = useRoundTimer();
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (!unref(isReady)) {
				...
				if (unref(lastResult)) {
					...
					_push(`<div class="scan__result-body" data-v-fb12b376><p class="scan__result-label" data-v-fb12b376>${ssrInterpolate(unref(checkinFeedback)?.kind === "invalid" ? "สแกนไม่สำเร็จ" : "สแกนสำเร็จ")}</p><p class="scan__result-value" data-v-fb12b376>${ssrInterpolate(unref(checkinFeedback)?.text || unref(lastResult))}</p></div>`);
					_push(ssrRenderComponent(_component_UButton, {
						size: "xs",
						variant: "soft",
						onClick: ($event) => {
			...
			_push(ssrRenderComponent(_component_UModal, {
				...
				body: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<div class="result-popup" data-v-fb12b376${_scopeId}>`);
						_push(ssrRenderComponent(_component_UIcon, {
							name: "i-lucide-party-popper",
							class: "result-popup__icon"
						}, null, _parent, _scopeId));
						_push(`<p class="result-popup__station" data-v-fb12b376${_scopeId}>${ssrInterpolate(unref(scannedStation)?.name)}</p>`);
						if (unref(scannedStation)) _push(`<p class="result-popup__points" data-v-fb12b376${_scopeId}> +${ssrInterpolate(unref(scannedStation).points)} Point </p>`);
						else _push(`<!---->`);
						_push(`</div>`);
			...
			_push(ssrRenderComponent(_component_UModal, {
				...
				footer: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<div class="result-popup__actions" data-v-fb12b376${_scopeId}>`);
						_push(ssrRenderComponent(_component_UButton, {
							block: "",
							color: "neutral",
							variant: "soft",
			...
			_push(ssrRenderComponent(_component_UModal, {
				...
				body: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<div class="survey-popup" data-v-fb12b376${_scopeId}><p class="survey-popup__question" data-v-fb12b376${_scopeId}>ท่านชอบด่านไหนมากที่สุด</p><div class="survey-popup__scale" data-v-fb12b376${_scopeId}><!--[-->`);
						ssrRenderList(SURVEY_RATING_OPTIONS, (option) => {
							_push(`<button type="button" class="${ssrRenderClass([{ "survey-popup__option--selected": unref(selectedRating) === option.value }, "survey-popup__option"])}" data-v-fb12b376${_scopeId}><span class="survey-popup__option-value" data-v-fb12b376${_scopeId}>${ssrInterpolate(option.value)}</span><span class="survey-popup__option-label" data-v-fb12b376${_scopeId}>${ssrInterpolate(option.label)}</span></button>`);
						});
						_push(`<!--]--></div>`);
						if (unref(surveyError)) _push(`<p class="survey-popup__error" data-v-fb12b376${_scopeId}>${ssrInterpolate(unref(surveyError))}</p>`);
						else _push(`<!---->`);
						_push(`</div>`);
...
//# sourceMappingURL=scan-K2-lHgDN.js.map