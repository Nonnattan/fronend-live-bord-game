export default defineNuxtConfig({
  ...
  pwa: {
    ...
    injectRegister: 'auto',
    // แก้ไขจุดนี้ (Dev Service Worker ดัก Vite HMR Asset — อีกสาเหตุหนึ่งของ
    // "Failed to fetch dynamically imported module" และ MIME mismatch ของ
    // *.vue?vue&type=style): เดิมเปิด Service Worker ใน `npm run dev` เสมอ
    // (enabled: true) เพื่อให้ทดสอบ Offline/Cache ได้จริงระหว่างพัฒนาโดยไม่ต้อง
    // build ทุกครั้ง — ปัญหาคือ Route ข้อ 4 ใน service-worker/sw.ts
    // (`request.destination === 'script' || 'style'`, StaleWhileRevalidate)
    // จะดักคำขอ /_nuxt/* ของ Vite HMR ทุกตัวไปด้วย (Vite เปลี่ยน URL ของไฟล์
    // เดิมด้วย Query String ใหม่ทุกครั้งที่แก้โค้ด เช่น `?v=xxxx` หรือ
    // `?vue&type=style&index=0` ที่ยังไม่เคยอยู่ใน Cache) ตัว Service Worker
    // จึงต้องคอย Fetch ของใหม่ไป Cache ควบคู่กับที่ Vite เองก็พยายาม Serve สด ๆ
    // อยู่แล้ว เกิด Race กัน — บาง Response ที่ Cache ไว้ผิดจังหวะเลย MIME Type
    // ไม่ตรงกับที่ Import ขอ (เจอเป็น CSS ทั้งที่ Import แบบ JS Module) และบาง
    // Request โดน Service Worker ตอบจาก Cache เก่าที่ยังไม่สมบูรณ์แทนที่จะปล่อย
    // ให้ Vite Serve ตรง ๆ กลายเป็น "Failed to fetch dynamically imported module"
    //
    // ปิดเป็นค่า Default ใน Dev ไปก่อน (ตรงกับพฤติกรรม Default ของ @vite-pwa/nuxt
    // เอง) แต่ยังเปิดให้ทดสอบ Offline/Cache ระหว่าง Dev ได้เหมือนเดิมถ้าต้องการ
    // จริง ๆ ผ่านการตั้ง Environment Variable `PWA_DEV_SW=true` ก่อนรัน
    // `npm run dev` (เช่น `PWA_DEV_SW=true npm run dev` หรือใส่ใน .env) — ไม่ได้
    // ลบความสามารถทดสอบ PWA ระหว่าง Dev ทิ้งไปเลย แค่ไม่ให้เปิดโดย Default จน
    // มารบกวน HMR ตามปกติ — Production (`npm run build` + `npm run preview` /
    // Deploy จริง) ไม่กระทบเลย เพราะ devOptions มีผลเฉพาะตอน Dev เท่านั้น
    devOptions: {
      enabled: process.env.PWA_DEV_SW === 'true',
      type: 'module',
...
vite: {
  server: {
    allowedHosts: true,
    // แก้ไขจุดนี้ (Vite HMR WebSocket Fix — สาเหตุของ
    // "[vite] failed to connect to websocket" / "Failed to fetch dynamically
    // imported module"): Nuxt dev server (Nitro) ฟังที่ port 3000 (public
    // endpoint ที่ Browser เปิดจริง) แต่ Vite dev server ภายใน (ที่ Nitro
    // proxy คำขอ HTTP ของ /_nuxt/* ไปหาให้) ฟังอยู่คนละ port คือ 5173 —
    // ปกติ Nitro จะ proxy ทั้ง HTTP และ WebSocket (สำหรับ HMR) ของ /_nuxt/*
    // จาก 3000 ไป 5173 ให้อัตโนมัติอยู่แล้ว แต่ Vite HMR Client (Script ที่ฝัง
    // มากับหน้าเว็บ) คำนวณ Port ที่จะเปิด WebSocket เองจาก `server.port`
    // ภายใน (5173) ไม่รู้จัก Port Public (3000) ที่ Browser ใช้อยู่จริง — เมื่อ
    // ต่อ ws://localhost:3000 ผ่าน Proxy ไม่สำเร็จ (หรือ Handshake ไม่ตรง)
    // Vite Client จะ Fallback ไปพยายามต่อตรงที่ ws://localhost:5173 เอง ซึ่ง
    // Browser จะปฏิเสธเพราะเป็นคนละ Origin/Port จาก Public Endpoint (Mixed
    // ต่อไม่ติดทั้งคู่) — ผลลัพธ์คือ HMR ตายสนิท และเมื่อ HMR/Vite Client
    // เชื่อมต่อไม่ได้ Browser จะยัง fetch Module แบบ Dynamic Import ต่อไปเรื่อย ๆ
    // จนบาง Request โดน Nitro/Vite ตัดจบกลางทาง (Outdated Request) กลายเป็น
    // "Failed to fetch dynamically imported module" และบางไฟล์ (โดยเฉพาะ
    // *.vue?vue&type=style) ถูกส่ง Response ผิด MIME Type กลับมาแทน
    //
    // แก้โดยบอก Vite HMR Client ตรง ๆ ว่า "ให้เปิด WebSocket ที่ Client-Side
    // ไปที่ Port 3000" (คนละเรื่องกับ server.port ภายในที่ยังคง 5173 เหมือนเดิม
    // ไม่ต้องย้าย) ผ่าน `server.hmr.clientPort` — Vite 8 เปลี่ยน API หลักไปใช้
    // `server.ws` แล้ว (ดู https://vite.dev/config/server-options.html#server-ws)
    // แต่ `server.hmr.*` (รวม clientPort) ยังใช้ได้ปกติเพราะ Vite Sync 2 ชุดนี้
    // ให้กันเองเสมอ (Deprecated แต่ไม่ใช่ Removed) — ใช้ `hmr` ต่อไปเพราะ Nuxt/
    // @nuxt/vite-builder ที่ project ใช้อยู่ (Nuxt 4.5.2) ยังรองรับ/Merge Key
    // นี้เข้ากับ Vite Config ภายในให้โดยตรงแน่นอนกว่า
    hmr: {
      protocol: 'ws',
      host: 'localhost',
      clientPort: 3000,
