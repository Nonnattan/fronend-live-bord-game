...
var evaluation_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		const stationChoices = computed(() => stations.value.map((s, index) => ({
			id: s.id,
			name: s.name,
		...
		*/
		async function handleSubmit() {
			...
			try {
				...
				const completedStations = stationChoices.value.filter((s) => isStationMissionComplete(s.id)).map((s) => ({
					name: s.name,
					points: 0
				}));
				const selectedStation = stationChoices.value.find((s) => s.id === selectedFavoriteStationId.value);
				if (isOfflineMode.value) endRound();
					...
					try {
						await submitSurvey({
							...
							favoriteStationRating: selectedStation?.order ?? 1
						});
					} catch (err) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "page" }, _attrs))} data-v-1e7f6ac8>`);
			_push(ssrRenderComponent(_component_PageHeader, {
				title: "แบบประเมิน",
				"back-to": "/stations"
			}, null, _parent));
			if (!unref(isReady)) {
				_push(`<div class="page__loading" data-v-1e7f6ac8>`);
				...
				_push(`<div class="page__content" data-v-1e7f6ac8><p class="page__question" data-v-1e7f6ac8>ท่านชอบฐานใดมากที่สุด?</p><div class="choice-list" data-v-1e7f6ac8><!--[-->`);
				ssrRenderList(unref(stationChoices), (choice) => {
					_push(`<button type="button" class="${ssrRenderClass([{ "choice--selected": unref(selectedFavoriteStationId) === choice.id }, "choice"])}" data-v-1e7f6ac8><span class="choice__icon" data-v-1e7f6ac8>${ssrInterpolate(choice.icon)}</span><span class="choice__name" data-v-1e7f6ac8>${ssrInterpolate(choice.name)}</span></button>`);
				});
				_push(`<!--]--></div>`);
...
var evaluation_default = /*#__PURE__*/ _plugin_vue_export_helper_default(evaluation_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-1e7f6ac8"]]);

export { evaluation_default as default };
//# sourceMappingURL=evaluation-B3O9KX7X.mjs.map
