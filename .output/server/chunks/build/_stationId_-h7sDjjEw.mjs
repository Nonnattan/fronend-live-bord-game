...
import { u as useStationMissions, a as useOfflineMissionAnswerSync } from './useStationMissions-CjHnM2MF.mjs';
import { u as useStationQuest } from './useStationQuest-DdT4jEnZ.mjs';
import { u as useOfflineSync } from './useOfflineSync-D8-RD8DK.mjs';
import { defineComponent, computed, ref, mergeProps, unref, withCtx, createTextVNode, isRef, watch, openBlock, createBlock, Fragment, renderList, toDisplayString, createCommentVNode, createVNode, readonly, useSSRContext } from 'vue';
...
var StationMissionCard_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		*/
		...
		const pointsLabel = computed(() => props.mission.completed ? `+${props.mission.points} คะแนน` : `${props.mission.points} คะแนน`);
		return (_ctx, _push, _parent, _attrs) => {
			const _component_UIcon = _sfc_main$7;
			_push(`<button${ssrRenderAttrs(mergeProps({
				type: "button",
				class: ["mission-card", { "mission-card--completed": __props.mission.completed }]
			}, _attrs))} data-v-e06c0c33><span class="mission-card__icon" data-v-e06c0c33>${ssrInterpolate(unref(icon))}</span><span class="mission-card__body" data-v-e06c0c33><span class="mission-card__title" data-v-e06c0c33>${ssrInterpolate(__props.mission.name)}</span><span class="mission-card__desc" data-v-e06c0c33>${ssrInterpolate(__props.mission.description)}</span><span class="mission-card__points" data-v-e06c0c33>${ssrInterpolate(unref(pointsLabel))}</span></span>`);
...
var StationMissionCard_default = /*#__PURE__*/ Object.assign(_plugin_vue_export_helper_default(StationMissionCard_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-e06c0c33"]]), { __name: "StationMissionCard" });
//#endregion
...
var StationMissionQuestion_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		* ---------------------------------------------------------------------------
		* [แก้ไข] ป็อปอัพคำถาม ใช้ร่วมกันทั้งด่าน SINGLE_QUESTION (1 คำถาม) และ
		* MULTI_QUESTION (N คำถาม — ไม่ hardcode จำนวน) — รับ `mission` ตรง ๆ (ด่านย่อย
		* ตอนนี้ self-contained เนื้อคำถาม/ตัวเลือกมาในตัวเองแล้ว ไม่ cross-reference กับ
		* ระบบคำถามเดิม/useQuestion.ts อีกต่อไป) ผู้เล่นเลือกตัวเลือกด้วย "index" เสมอ
		* (ไม่ใช่ข้อความ) — emit ออกไปเป็น array ของ index (ยาว 1 สำหรับ SINGLE_QUESTION,
		* ยาว N สำหรับ MULTI_QUESTION) ตัวหน้าเต็มเป็นคนเลือกว่าจะเรียก
		* useStationMissions().submitSingleQuestion()/submitMultiQuestion() ตัวไหนตอน
		* @submit ไม่ตัดสิน/คำนวณคะแนนเองที่นี่เลย แค่รับ `result` กลับมาแสดงผล
		*
		* `result.pending` = true หมายถึงยังไม่รู้ผล (ไม่มีเน็ตตอนส่ง ถูก queue ไว้รอ
		* sync จริง — ดู composables/useStationMissions.ts) ต้องรอเน็ตกลับมาก่อนถึงจะรู้
		* ถูก/ผิด+คะแนนจริง (Backend ไม่ส่งเฉลยมาให้ฝั่งนี้ตัดสินเองได้อีกต่อไป)
		*/
		const props = __props;
		const emit = __emit;
		/** รายการ "ข้อ" ที่ต้องแสดง — SINGLE_QUESTION มี 1 ข้อเสมอ (จาก mission.question/
		* options ตรง ๆ) MULTI_QUESTION มี N ข้อจาก mission.questions[] */
		const items = computed(() => {
			if (!props.mission) return [];
			if (props.mission.type === "MULTI_QUESTION") return props.mission.questions ?? [];
			return [{
				question: props.mission.question ?? "",
				options: props.mission.options ?? []
			}];
		});
		const localAnswers = ref([]);
		watch(() => props.mission?.id, () => {
			localAnswers.value = items.value.map(() => -1);
		}, { immediate: true });
		const hasResult = computed(() => !!props.result);
		const isMulti = computed(() => items.value.length > 1);
		const allAnswered = computed(() => localAnswers.value.every((a) => a >= 0) && localAnswers.value.length === items.value.length);
		function selectChoice(itemIndex, optionIndex) {
			if (hasResult.value) return;
			const next = [...localAnswers.value];
			next[itemIndex] = optionIndex;
			localAnswers.value = next;
		}
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(_component_UModal, mergeProps({
				...
				body: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						if (!unref(hasResult)) {
							_push(`<div class="mission-question" data-v-4c8228a9${_scopeId}><!--[-->`);
							ssrRenderList(unref(items), (item, idx) => {
								_push(`<div class="mission-question__item" data-v-4c8228a9${_scopeId}>`);
								if (unref(isMulti)) _push(`<p class="mission-question__index" data-v-4c8228a9${_scopeId}>ข้อ ${ssrInterpolate(idx + 1)} / ${ssrInterpolate(unref(items).length)}</p>`);
								else _push(`<!---->`);
								_push(`<p class="mission-question__text" data-v-4c8228a9${_scopeId}>${ssrInterpolate(item.question)}</p><div class="mission-question__choices" data-v-4c8228a9${_scopeId}><!--[-->`);
								ssrRenderList(item.options, (option, optIdx) => {
									_push(`<button type="button" class="${ssrRenderClass([{ "mission-question__choice--selected": unref(localAnswers)[idx] === optIdx }, "mission-question__choice"])}" data-v-4c8228a9${_scopeId}>${ssrInterpolate(option)}</button>`);
								});
								_push(`<!--]--></div></div>`);
							});
							_push(`<!--]--></div>`);
						} else {
							_push(`<div class="mission-question__result" data-v-4c8228a9${_scopeId}>`);
							if (__props.result.pending) {
								_push(`<!--[-->`);
								_push(ssrRenderComponent(_component_UIcon, {
									name: "i-lucide-cloud-off",
									class: "mission-question__result-icon"
								}, null, _parent, _scopeId));
								_push(`<p class="mission-question__result-summary" data-v-4c8228a9${_scopeId}> ส่งคำตอบไว้แล้ว รอเชื่อมต่ออินเทอร์เน็ตเพื่อตรวจผล </p><!--]-->`);
							} else if (__props.result.error) _push(`<p class="mission-question__result-summary" data-v-4c8228a9${_scopeId}>${ssrInterpolate(__props.result.error)}</p>`);
							else {
								_push(`<!--[-->`);
								if (__props.result.pointsAwarded > 0) _push(ssrRenderComponent(_component_UIcon, {
									name: "i-lucide-check-circle-2",
									class: "mission-question__result-icon mission-question__result-icon--correct"
								}, null, _parent, _scopeId));
								else _push(`<!---->`);
								if (__props.result.totalQuestions && __props.result.totalQuestions > 1) _push(`<p class="mission-question__result-summary" data-v-4c8228a9${_scopeId}> ตอบถูก ${ssrInterpolate(__props.result.correctCount)} / ${ssrInterpolate(__props.result.totalQuestions)} ข้อ </p>`);
								else _push(`<!---->`);
								if (__props.result.pointsAwarded > 0) _push(`<p class="mission-question__result-points" data-v-4c8228a9${_scopeId}> +${ssrInterpolate(__props.result.pointsAwarded)} คะแนน </p>`);
								else _push(`<p class="mission-question__result-points mission-question__result-points--wrong" data-v-4c8228a9${_scopeId}> ยังไม่ผ่านด่านนี้ — ลองใหม่ได้ </p>`);
								_push(`<!--]-->`);
							}
							_push(`</div>`);
						}
						...
						return openBlock(), createBlock("div", {
							...
							}, "ข้อ " + toDisplayString(idx + 1) + " / " + toDisplayString(unref(items).length), 1)) : createCommentVNode("", true),
							createVNode("p", { class: "mission-question__text" }, toDisplayString(item.question), 1),
							createVNode("div", { class: "mission-question__choices" }, [(openBlock(true), createBlock(Fragment, null, renderList(item.options, (option, optIdx) => {
								return openBlock(), createBlock("button", {
									key: optIdx,
									type: "button",
									class: ["mission-question__choice", { "mission-question__choice--selected": unref(localAnswers)[idx] === optIdx }],
									onClick: ($event) => selectChoice(idx, optIdx)
					...
					}, [__props.result.pending ? (openBlock(), createBlock(Fragment, { key: 0 }, [createVNode(_component_UIcon, {
						name: "i-lucide-cloud-off",
						class: "mission-question__result-icon"
					}), createVNode("p", { class: "mission-question__result-summary" }, " ส่งคำตอบไว้แล้ว รอเชื่อมต่ออินเทอร์เน็ตเพื่อตรวจผล ")], 64)) : __props.result.error ? (openBlock(), createBlock("p", {
						key: 1,
						class: "mission-question__result-summary"
					}, toDisplayString(__props.result.error), 1)) : (openBlock(), createBlock(Fragment, { key: 2 }, [
						__props.result.pointsAwarded > 0 ? (openBlock(), createBlock(_component_UIcon, {
							key: 0,
							name: "i-lucide-check-circle-2",
							class: "mission-question__result-icon mission-question__result-icon--correct"
						})) : createCommentVNode("", true),
						__props.result.totalQuestions && __props.result.totalQuestions > 1 ? (openBlock(), createBlock("p", {
							key: 1,
							class: "mission-question__result-summary"
						}, " ตอบถูก " + toDisplayString(__props.result.correctCount) + " / " + toDisplayString(__props.result.totalQuestions) + " ข้อ ", 1)) : createCommentVNode("", true),
						__props.result.pointsAwarded > 0 ? (openBlock(), createBlock("p", {
							key: 2,
							class: "mission-question__result-points"
						}, " +" + toDisplayString(__props.result.pointsAwarded) + " คะแนน ", 1)) : (openBlock(), createBlock("p", {
							key: 3,
							class: "mission-question__result-points mission-question__result-points--wrong"
						}, " ยังไม่ผ่านด่านนี้ — ลองใหม่ได้ "))
					], 64))]))];
				}),
...
var StationMissionQuestion_default = /*#__PURE__*/ Object.assign(_plugin_vue_export_helper_default(StationMissionQuestion_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-4c8228a9"]]), { __name: "StationMissionQuestion" });
//#endregion
...
var _stationId__vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		const isFinalStation = computed(() => station.value?.isFinal === true);
		...
		/** ผู้เล่นกดส่งคำตอบใน StationMissionQuestion — ยิงตรงไปให้ Backend ตัดสินทันที
		* (submitSingleQuestion()/submitMultiQuestion() พยายามยิง network ก่อนเสมอ ไม่มี
		* เน็ตจริง ๆ ค่อย queue ไว้รอ sync — ดู composables/useStationMissions.ts) แล้ว
		* อัปเดต activeQuestionResult ให้ Popup เปลี่ยนไปแสดงผลลัพธ์เอง */
		async function handleQuestionSubmit(answers) {
			if (!stationId.value || !activeMission.value) return;
			const ctx = currentCtx();
			const outcome = activeMission.value.type === "SINGLE_QUESTION" ? await submitSingleQuestion(stationId.value, activeMission.value, answers[0] ?? -1, ctx) : await submitMultiQuestion(stationId.value, activeMission.value, answers, ctx);
		...
		* ออนไลน์เสมอ (ไม่มี Offline Fallback) ยิงไม่สำเร็จ (QR ผิด/หมดเน็ต) -> แสดง
		* error ให้สแกนใหม่ได้ ไม่ถือว่าสำเร็จ/ได้คะแนนก่อนได้รับคำตอบจริง */
		...
		* อัตโนมัติทันทีที่ roundId เปลี่ยน ไม่ต้องรีเซ็ตเองที่นี่ */
		...
		/** ปุ่ม "จบเกม" — กดได้ไม่ว่าทำด่านย่อยได้กี่ใบก็ตาม เปลี่ยนแค่ gameCompleted เป็น
		* true เท่านั้น (ดู markGameCompleted()) ไม่แตะด่านย่อย/คะแนนเด็ดขาด แล้วพาไปหน้า
		...
		* เป็นคนจบ Round จริงหลังส่งแบบประเมินแล้วเท่านั้น) */
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			const _component_UIcon = _sfc_main$7;
			const _component_UButton = _sfc_main$2;
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "page" }, _attrs))} data-v-92db6c29>`);
			_push(ssrRenderComponent(_component_PageHeader, {
				title: unref(stationName) ? `ภารกิจ${unref(stationName)}` : "ภารกิจฐาน",
				"back-to": "/stations"
			}, null, _parent));
			if (!unref(isReady) || !unref(canShowContent)) {
				_push(`<div class="page__loading" data-v-92db6c29>`);
				_push(ssrRenderComponent(_component_UIcon, {
				...
				_push(`<div class="page__content" data-v-92db6c29>`);
				if (unref(loadingMissions) && unref(missions).length === 0) {
					...
					_push(`<div class="mission-error" data-v-92db6c29><p class="mission-error__text" data-v-92db6c29>${ssrInterpolate(unref(missionsLoadError))}</p>`);
				...
				if (unref(isFinalStation)) {
					_push(`<div class="endgame-cta" data-v-92db6c29><p class="endgame-cta__title" data-v-92db6c29>${ssrInterpolate(unref(isComplete) ? "ทำด่านย่อยของฐานนี้ครบแล้ว!" : "ต้องการจบเกมตอนนี้เลยหรือไม่?")}</p>`);
					_push(ssrRenderComponent(_component_UButton, {
						block: "",
						size: "xl",
						color: "neutral",
			...
			if (unref(activeMission) && unref(activeMission).type === "QR_SCORE") _push(ssrRenderComponent(StationMissionQr_default, {
				...
				completed: unref(activeMission).completed ?? false,
				"points-earned": unref(activeMission).points,
...
var _stationId__default = /*#__PURE__*/ _plugin_vue_export_helper_default(_stationId__vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-92db6c29"]]);

export { _stationId__default as default };
//# sourceMappingURL=_stationId_-h7sDjjEw.mjs.map
