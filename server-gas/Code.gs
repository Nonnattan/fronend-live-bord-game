...
function actionPing_() {
  ...
  // ไฟล์ใหม่ (รอบนี้): ระบบ "ภารกิจของฐาน" — ดู MissionsService.gs (Content-Driven
  // จากชีต Questions ตอนนี้ ไม่มีชีตของตัวเอง ยกเว้น "MissionCompletions" ที่ใช้
  // เฉพาะภารกิจ QR_SCORE — เช็ค missionCompletionsSheetFound หลัง redeploy ทุกครั้ง
  // เพื่อยืนยันว่าโค้ดเวอร์ชันใหม่ถูก deploy จริงแล้ว)
  ...
  info.missionAnswersServiceLoaded =
    typeof actionSubmitMissionAnswers_ === "function";
  info.missionCompletionsSheetFound = !!ss.getSheetByName("MissionCompletions");
...
function handleRequest_(payload) {
  ...
  try {
    switch (action) {
      ...
      // ไฟล์ใหม่ (รอบนี้): ระบบ "ภารกิจของฐาน" — ดู MissionsService.gs (Content-
      // Driven จากชีต Questions จริงผ่านคอลัมน์ MissionKey — ไม่ hardcode จำนวน
      // ด่านอีกต่อไป) listStationMissions ไม่เขียนข้อมูล verifyMissionQr/
      // submitMissionAnswers ให้คะแนนจริง (บันทึกลง MissionCompletions/Answers
      // ตามชนิดภารกิจ) — verifyMissionQr ใช้คอลัมน์ "MissionQrToken" ในชีต
      // Stations แยกขาดจาก "QrToken" เดิมของ verifyStationQr เด็ดขาด
      ...
      case "submitMissionAnswers":
        return jsonOutput_(actionSubmitMissionAnswers_(payload));
