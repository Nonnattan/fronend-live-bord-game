 ...
 * - MissionQrPoints : ไฟล์ใหม่ — คะแนนของภารกิจ "สแกน QR" (type: QR_SCORE) ของฐานนี้
 *                คนละก้อนกับ Points ด้านบน (Points = แต้มฐานเดิม ไม่เกี่ยวกับภารกิจ
 *                ตามสเปกใหม่ที่ Station Scan ไม่ให้คะแนนแล้ว) ไม่บังคับต้องกรอก —
 *                ว่าง/ไม่ใช่ตัวเลข fallback เป็น 10 เสมอ (ไม่ต้อง backfill เขียนค่า
 *                ลงชีตเหมือน QrToken เพราะเป็นแค่ตัวเลขธรรมดา ไม่ต้อง unique — Admin
 *                ปรับเปลี่ยนได้จากหน้า Admin โดยไม่ต้อง Deploy โค้ดใหม่)
 */
...
// ตำแหน่งคอลัมน์ (0-indexed) ของ QrToken/MissionQrToken/MissionQrPoints — ใช้
// indexOf แทนเลขคอลัมน์ตรง ๆ (เดิมใช้ STATIONS_HEADERS.length - 1 ตอน QrToken ยังเป็น
// คอลัมน์สุดท้าย — ใช้ต่อไม่ได้แล้วหลังเพิ่มคอลัมน์ต่อท้ายอีก) กัน bug ตำแหน่งเลื่อนถ้ามี
...
var STATIONS_MISSION_QR_POINTS_COL_INDEX =
  STATIONS_HEADERS.indexOf("MissionQrPoints");
/** ค่าเริ่มต้นของ MissionQrPoints ถ้าฐานยังไม่ได้กรอก (แถวเก่า/สร้างใหม่ไม่ระบุ) */
const DEFAULT_MISSION_QR_POINTS_ = 10;

/** คืนค่าชีต "Stations" — สร้างชีตใหม่ + ใส่หัวตารางให้อัตโนมัติถ้ายังไม่มี
 * (ไม่แตะต้องชีตอื่นใดในสเปรดชีตเดียวกันเลย)
 *
 * [แก้ไข] เดิมใช้ SpreadsheetApp.getActiveSpreadsheet() ต่างจากทุก Service อื่น
 * (Members/Journey/Score/Round ล้วนแก้เป็น openById(SPREADSHEET_ID) ไปแล้วก่อนหน้า
 * นี้ตามคอมเมนต์ของตัวเอง) — ถ้าโปรเจกต์ Apps Script ถูก deploy จากที่ไม่ผูก
 * (bound) กับสเปรดชีตนี้ตรง ๆ getActiveSpreadsheet() จะคืน null/ชี้ผิดสเปรดชีตทันที
 * แก้ให้ตรงกับ Service อื่นทุกไฟล์ เพราะ MissionsService.gs จะเรียกฟังก์ชันนี้ทุก
 * ครั้งที่มีการเรียกภารกิจของฐานแล้ว (listStationMissions/verifyMissionQr) */
...
function rowToStation_(row) {
  return {
    ...
    missionQrToken: row[STATIONS_MISSION_QR_TOKEN_COL_INDEX]
      ...
      : "",
    // ไฟล์ใหม่: ไม่ backfill เขียนค่าลงชีต (ไม่ต้อง unique เหมือน QrToken) — แค่
    // fallback ตอนอ่านเป็น DEFAULT_MISSION_QR_POINTS_ ถ้าแถวนี้ยังไม่ได้กรอก/ไม่ใช่
    // ตัวเลขที่ใช้ได้ (แนวทางเดียวกับ imageUrl/type/lat/lng ด้านบน)
    missionQrPoints:
      row[STATIONS_MISSION_QR_POINTS_COL_INDEX] === "" ||
      row[STATIONS_MISSION_QR_POINTS_COL_INDEX] === undefined ||
      row[STATIONS_MISSION_QR_POINTS_COL_INDEX] === null ||
      isNaN(Number(row[STATIONS_MISSION_QR_POINTS_COL_INDEX]))
        ? DEFAULT_MISSION_QR_POINTS_
        : Number(row[STATIONS_MISSION_QR_POINTS_COL_INDEX]),
  };
}
...
function createStation_(sheet, input, now) {
  ...
  const newRow = [
    ...
    qrToken,
    missionQrToken,
    input.missionQrPoints === undefined ||
    input.missionQrPoints === null ||
    input.missionQrPoints === ""
      ? DEFAULT_MISSION_QR_POINTS_
      : Number(input.missionQrPoints) || 0,
  ];
...
function updateStation_(sheet, rowIndex, input, now) {
  ...
  const updatedRow = [
    ...
    // ไฟล์ใหม่: MissionQrPoints แก้ไขได้อิสระเหมือน points ปกติ (ไม่ต้อง unique)
    input.missionQrPoints !== undefined
      ? input.missionQrPoints === null || input.missionQrPoints === ""
        ? DEFAULT_MISSION_QR_POINTS_
        : Number(input.missionQrPoints) || 0
      : current[STATIONS_MISSION_QR_POINTS_COL_INDEX],
  ];
  sheet
    .getRange(rowIndex, 1, 1, STATIONS_HEADERS.length)
