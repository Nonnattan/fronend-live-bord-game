 ...
 *
 * [แก้ไข — เอาชีตแคช "RoundScoreCache" ออกทั้งหมด] เดิม actionGetRewardStatus_
 * รับ payload.score จาก Client มา "เชื่อตรง ๆ" แล้วแคชไว้ที่นี่ ให้
 * actionClaimReward_/actionListPendingRewards_ อ่านก่อนเสมอ (ทางลัดชั่วคราวช่วง
 * Demo ที่คอมเมนต์เดิมเตือนไว้เองแล้วว่า "ไม่แนะนำสำหรับ Production จริง") — พบว่า
 * เป็นช่องโหว่ที่ใช้งานอยู่จริง ไม่ใช่แค่ทฤษฎี: pages/round-summary.vue ส่ง
 * mockScore ของระบบภารกิจ Mock เดิม (useStationQuest — ตอนนี้ถูกแทนที่ด้วยภารกิจ
 * จริงที่คะแนนมาจาก Backend เท่านั้นแล้ว) มาเป็น score นี้ทุก 10 วินาที ทำให้สิทธิ์
 * รางวัลจริงถูกตัดสินจากตัวเลขที่ Client ควบคุมได้ ไม่ใช่ Journey+Answers จริง —
 * ตอนนี้กลับไป "เชื่อ server ล้วน ๆ" ตามที่คอมเมนต์เดิมเองก็บอกว่าทำได้เมื่อพร้อม
 * (คำนวณจาก computeRoundScore_ เสมอ ไม่มีทางลัดอีกต่อไป) — ชีต "RoundScoreCache"
 * ที่อาจมีแถวเก่าค้างอยู่จากก่อนหน้านี้ไม่ถูกลบทิ้ง (ไม่กระทบอะไร แค่ไม่ถูกใช้
 * งานอีกต่อไป)
 */
 ...
 * ผลรวมทั้งสามก้อน (Journey + Answers + MissionCompletions — ดูข้อ 3 ด้านล่าง)
 * = คะแนนรวมที่ใช้ตัดสินสิทธิ์รางวัล ตรงกับที่หน้า pages/round-summary.vue แสดง
 * "คะแนนรวม" ให้ผู้เล่นเห็นทุกประการ จึงไม่ต้องให้เจ้าหน้าที่พิมพ์คะแนนเองที่หน้า
 * /redeem เลย (กรอกแค่ roundId + userId ที่อ่านจากหน้าจอผู้เล่นก็พอ)
 *
 * [ใหม่] 3) ชีต "MissionCompletions" (MissionsService.gs) — ผลรวมคอลัมน์
 * PointsEarned ของภารกิจ QR_SCORE ที่ทำสำเร็จในรอบนี้ (ภารกิจ SINGLE_QUESTION/
 * MULTI_QUESTION นับรวมอยู่ในก้อน Answers ด้านบนแล้ว ไม่ปนกับก้อนนี้)
 */
function computeRoundScore_(roundId, userId) {
  ...
  for (let i = 0; i < completionRows.length; i++) {
    const completion = rowToMissionCompletion_(completionRows[i]);
    if (completion.roundId === rid && completion.userId === uid)
      total += completion.pointsEarned;
  }
...
function getRoundScoresBreakdown_(roundId, userId) {
  ...
  // [ใหม่] แต้มภารกิจ QR_SCORE จากชีต "MissionCompletions" (MissionsService.gs)
  // — คนละก้อนกับ point (Journey)/questionPoint (Answers) เสมอ ไม่มีการนับซ้ำ
  const completionRows = getAllMissionCompletionRows_(
    getMissionCompletionsSheet_(),
  );
  for (let i = 0; i < completionRows.length; i++) {
    const completion = rowToMissionCompletion_(completionRows[i]);
    if (completion.roundId !== rid || completion.userId !== uid) continue;
    const row = ensureStation_(completion.stationId, null);
    row.missionQrPoint += completion.pointsEarned;
  }
  ...
  const totalMissionQrPoint = stations.reduce(function (sum, s) {
    return sum + s.missionQrPoint;
  }, 0);
  ...
  return {
    ...
    totalMissionQrPoint: totalMissionQrPoint,
    totalScore: totalPoint + totalQuestionPoint + totalMissionQrPoint,
  };
}
 ...
 *
 * [แก้ไข] ไม่รับ/ไม่เชื่อ payload.score จาก Client อีกต่อไป (เดิมเป็นช่องโหว่จริง
 * — ดูคำอธิบายเต็ม ๆ ที่หัวไฟล์) คำนวณจาก computeRoundScore_ (Journey+Answers+
 * MissionCompletions ฝั่ง server) เสมอทุกครั้ง ไม่มีทางลัดอีกต่อไป
 */
...
function actionClaimReward_(payload) {
  ...
  // [แก้ไข] ไม่มีทางลัดจากแคชอีกต่อไป — คำนวณจาก Journey+Answers+MissionCompletions
  // ฝั่ง server เสมอ (ดูคำอธิบายเต็ม ๆ ที่หัวไฟล์ + actionGetRewardStatus_)
  const score = computeRoundScore_(roundId, userId);
...
function actionListPendingRewards_() {
  ...
  for (let i = 0; i < roundRows.length; i++) {
    ...
    // [แก้ไข] เหมือน actionClaimReward_ — ไม่มีทางลัดจากแคชอีกต่อไป คำนวณจาก
    // server เสมอ (ดูคำอธิบายเต็ม ๆ ที่หัวไฟล์ + actionGetRewardStatus_)
    const score = computeRoundScore_(entry.roundId, entry.userId);
    const tier = findQualifyingRewardTier_(tiers, score);
...
function actionGetRoundScores_(payload) {
  ...
  return {
    ...
    totalMissionQrPoint: breakdown.totalMissionQrPoint,
