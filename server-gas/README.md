...
## เบอร์โทรศัพท์ต้องเก็บเป็น "ข้อความ" เสมอ (แก้บั๊กเลข 0 นำหน้าหาย)

**อาการเดิม**: สมัคร/Login ครั้งแรกแล้วเบอร์ `0812345678` กลายเป็นตัวเลข
`812345678` ในชีต (เลข 0 หาย) แต่พอกด "แก้ไขโปรไฟล์" กลับเก็บถูกต้องมี 0 ครบ

**สาเหตุ**: `sheet.appendRow()` ที่ใช้ตอนสร้างสมาชิกใหม่ (`createMemberRow_`)
**ไม่เคารพ number format `'@'`** ที่ `ensurePhoneColumnIsText_()` ตั้งไว้ล่วงหน้า —
มันตีความค่าเหมือนผู้ใช้พิมพ์เองในชีต จึงแปลงเป็นตัวเลขทันที ต่างจาก
`setValue()`/`setValues()` (ที่ `actionUpdateMember_` ใช้) ซึ่งเคารพ format
และเก็บเป็น string ตามที่ส่งไปจริง — ตรงกับอาการที่พบพอดี

**สิ่งที่แก้**:
- `normalizePhone_()` (ใหม่) — แปลงเป็น string, ตัดอักขระที่ไม่ใช่ตัวเลขทิ้ง
  (รองรับ `081-234-5678`), เติมเลข 0 นำหน้ากลับถ้าเหลือ 9 หลักและยังไม่ขึ้นต้นด้วย 0
- `writePhoneAsText_()` (ใหม่) — ตั้ง format `'@'` ที่เซลล์นั้นแล้วเขียนด้วย `setValue()`
- `createMemberRow_()` — เรียก `writePhoneAsText_()` ทับทันทีหลัง `appendRow()`
- `rowToMember_()` — คืนค่าเบอร์ผ่าน `normalizePhone_()` ทำให้**แถวเก่าที่เสียไปแล้ว
  ถูกอ่านกลับมาถูกต้อง**ทันทีโดยไม่ต้องแก้ในชีต
- `findRowIndexByPhone_()` — เทียบด้วย `normalizePhone_()` ทั้งสองฝั่ง แก้บั๊กเงียบที่
  หาสมาชิกเดิมไม่เจอ (เทียบ `"0812345678"` กับ `812345678`) แล้ว **สร้างแถวซ้ำ**

**ซ่อมข้อมูลเก่าในชีต** (ตัวเลขที่เก็บผิดไปแล้ว): เปิด Apps Script Editor เลือก
ฟังก์ชัน `repairPhoneNumbers` จาก dropdown แล้วกด **Run** — จะไล่แก้ทุกแถวที่เบอร์
ผิดรูปแบบให้เป็น Text ที่ถูกต้อง (ข้ามแถวที่ถูกอยู่แล้ว รันซ้ำได้ ไม่มีผลข้างเคียง)
**แนะนำให้ทำสำเนาชีตไว้ก่อน** (File > Make a copy)
